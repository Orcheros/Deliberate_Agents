"""Threads provider using Unipile API.

Implements unified SocialProvider interface for Meta Threads.
Character limit: 500. Hashtags are not effectively used on this platform.
"""
...
class ThreadsProvider(SocialProvider):
    """Threads provider using Unipile API (https://unipile.com)."""
    ...
    def __init__(self, api_key: str = None, account_id: str = None):
        ...
        self.account_id = account_id or os.environ.get("UNIPILE_THREADS_ACCOUNT_ID")
        if not self.api_key:
            raise ValueError("UNIPILE_API_KEY environment variable required")
        if not self.account_id:
            raise ValueError("UNIPILE_THREADS_ACCOUNT_ID environment variable required")
    ...
    def publish_post(self, content: str, media_urls: Optional[list[str]] = None, metadata: Optional[dict] = None) -> dict:
        if len(content) > self.CHAR_LIMIT:
            raise ValueError(f"Content exceeds Threads character limit: {len(content)}/{self.CHAR_LIMIT}")
        ...
        for post in result.get("items", []):
            posts.append({
                ...
                "metrics": {
                    ...
                    "shares": post.get("metrics", {}).get("reposts", 0),
                },
            })
        return posts
